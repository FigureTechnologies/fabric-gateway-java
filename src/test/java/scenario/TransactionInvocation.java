...
public final class TransactionInvocation {
    ...
    private void invoke(Callable<TransactionResponse> invocationFn) {
        try {
            TransactionResponse result = invocationFn.call();
            setResponse(result);
        } catch (Exception e) {
            setError(e);
        }
    ...
    private void setResponse(TransactionResponse response) {
        String text = ScenarioSteps.newString(response.getPayload());
